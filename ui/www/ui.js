...
$('[name=ropel]').on('click', function() {
	NetworkTables.putValue("/SmartDashboard/align", 225)
...
$('[name=roper]').on('click', function() {
	NetworkTables.putValue("/SmartDashboard/align", 45)
	$(this).addClass('pressed');
	$('.airship_aligning').css('display', 'block')
	var $el = $(this);
	setTimeout(function() {
...
$('.auto').on('change', function() {
   console.log('autonomous: ', $('.auto:checked').val())
   var autoMode = parseInt($('.auto:checked').val());
	NetworkTables.putValue("/SmartDashboard/autonomousMode", autoMode);
});